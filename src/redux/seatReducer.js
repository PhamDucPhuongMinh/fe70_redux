...
export const seatReducer = (state = stateDefault, action) => {
  switch (action.type) {
    case "THEM_XOA_GHE_DANG_DAT": {
      let indexGheDangDat = state.danhSachGheDangDat.find(
        (item) => item.soGhe === action.ghe.soGhe
      );

      let [...danhSachGheDangDatMoi] = state.danhSachGheDangDat;
      if (indexGheDangDat) {
        danhSachGheDangDatMoi = danhSachGheDangDatMoi.filter(
          (item) => item.soGhe !== action.ghe.soGhe
        );
      } else {
        danhSachGheDangDatMoi.push(action.ghe);
      }
      state.danhSachGheDangDat = danhSachGheDangDatMoi;
      return { ...state };
    }
